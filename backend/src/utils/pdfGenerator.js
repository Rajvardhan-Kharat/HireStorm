...
exports.generateCampusOfferLetterPDF = async ({ studentName, role, collegeName, companyName, startDate, endDate, stipend, uniqueId }) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.on('end', async () => {
        try {
          ...
          const result = await uploadBufferToCloudinary(pdfData, `campus_offer_${uniqueId}`);
      ...
      const org = companyName || 'HireStorm';
      ...
      doc.fontSize(10.5).fillColor('#1e293b').font('Helvetica')
        .text(
          `We are pleased to extend this formal offer of internship to you from ${org}, selected through the campus placement drive conducted at ${collegeName || 'your college'}. After a thorough evaluation — including your academic credentials, AI skills assessment, and ${doc._infoFields?.interview ? 'interview performance' : 'application review'} — we believe you are an excellent fit for the role.`,
      ...
      // ── Signature Area ────────────────────────────────────────────────────────
      doc.moveDown(2);
      doc.fontSize(10).fillColor('#0f172a').font('Helvetica-Bold').text('For HireStorm / Innobytes');
      doc.moveDown(0.3);
      ...
      doc.fontSize(9).text('Campus Placement Division, HireStorm');
